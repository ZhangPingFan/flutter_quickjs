...
# directories
SOURCE="../cxx/quickjs"
SOURCE_URL="https://bellard.org/quickjs/quickjs-2020-11-08.tar.xz"
FAT="QuickJS_iOS"
THIN=`pwd`/"thin"
...
then
	...
	do
		...
		TMPDIR=${TMPDIR/%\/} CONFIG_M32=${CONFIG_M32} CONFIG_EXTRA_CFLAGS=${CONFIG_EXTRA_CFLAGS} CONFIG_XCRUN_EXTRA_FLAG=${CONFIG_XCRUN_EXTRA_FLAG} make -f ../../ios/Makefile -C $CWD/$SOURCE clean libquickjs.a || exit 1
...
then
	...
	rm -rf ./framework
	mkdir -p framework
	mkdir -p $CWD/$SOURCE/../headers
	CWD=`pwd`
	cp $CWD/$SOURCE/quickjs*.h $CWD/$SOURCE/../headers
	...
	for ARCH in $ARCHS
	do
		BUILD_FW="$BUILD_FW -library $THIN/$ARCH/lib/libquickjs.a -headers $CWD/$SOURCE/../headers"
	done
	BUILD_FW="$BUILD_FW -output framework/QuickJS.xcframework"
	echo $BUILD_FW
	$BUILD_FW
fi
